...
export interface ControllerState { tick: number; ready: boolean; lastCommand?: string; error?: string; }
...
export function createPillBottleController(gameId: string, receive: (state: ControllerState) => void) {
  ...
  const publish = (error?: string) => receive({ tick, ready, lastCommand, error });

  function loop(now: number) {
    if (destroyed || !ready) return;
    if (lastTime === undefined) lastTime = now;
    accumulator += Math.min(now - lastTime, 250); lastTime = now;
    while (accumulator >= 1000 / 60) { tick += 1; accumulator -= 1000 / 60; }
    if (tick % 15 === 0 && tick !== lastProgressTick) { lastProgressTick = tick; void set(ref(realtimeDatabase!, `games/${gameId}/players/${playerId}/progress`), {
      epochId, tick, lastClientSeq: clientSeq, phase: 'controller', serverTime: serverTimestamp()
    }); }
  ...
  const unsubscribe: Unsubscribe = onValue(ref(realtimeDatabase, `games/${gameId}/start`), async (snapshot) => {
    ...
    const start = snapshot.val() as { players?: Record<string, unknown>; tickRate?: number };
    ...
    ready = true; publish(); frame = requestAnimationFrame(loop);
