...
export async function getRoom(code: string) {
  ...
  return { id: snapshot.id, ...snapshot.data() } as { id: string; hostUid: string; status: 'lobby' | 'active'; ruleset: 'tetris' | 'doctor' | 'quarry' | 'canopy' | 'stax'; activeGameId?: string };
}

export async function updateRoomRuleset(code: string, ruleset: 'tetris' | 'doctor' | 'quarry' | 'canopy' | 'stax') {
...
export function subscribeRoom(roomId: string, receive: (room: { status: 'lobby' | 'active'; ruleset: 'tetris' | 'doctor' | 'quarry' | 'canopy' | 'stax'; activeGameId?: string }) => void, fail: (error: Error) => void): Unsubscribe {
  if (!firestore) throw new Error('Firebase is not configured.');
  return onSnapshot(doc(firestore, 'rooms', roomId),
    (snapshot) => { if (snapshot.exists()) receive(snapshot.data() as { status: 'lobby' | 'active'; ruleset: 'tetris' | 'doctor' | 'quarry' | 'canopy' | 'stax'; activeGameId?: string }); }, fail);
}
