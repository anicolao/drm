...
{:else}<main class="landscape-controller" class:gamepad-mode={gamepadActive} aria-label="Pill Bottle controller"><GameAudio state={state.bottle} enabled={state.audioOutput==='controllers'} rainSignal={state.rainSignal??0}/>
  {#if state.lifecycle}<aside class="controller-scoreboard" aria-label="Scores"><strong>RACE TO 3</strong>{#each standings as player}<span class:you={player.playerId===state.playerId}>{player.name} <b>{player.score}</b>{#if player.roundPoints>0}<small>+{player.roundPoints}</small>{/if}</span>{/each}</aside>{/if}
  ...
  {#if state.lifecycle?.finished}<MatchResult title={state.lifecycle.matchComplete?'RACE COMPLETE':state.lifecycle.winnerId===state.playerId?'ROUND WIN':'ROUND COMPLETE'} action={state.lifecycle.matchComplete?'PLAY AGAIN':'NEXT LEVEL'} ready={state.lifecycle.readyPlayerIds.length} total={state.lifecycle.playerIds.length} disabled={state.lifecycle.readyPlayerIds.includes(state.playerId??'')} level={selectedLevel} changeLevel={(level)=>selectedLevel=level} activate={nextRound}>{#if standings.length>0}<MatchStandings entries={standings}/>{/if}</MatchResult>{/if}
