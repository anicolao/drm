<script lang="ts">
  ...
  $: soloCleared=solo&&state.lifecycle?.terminalResults[state.playerId??'']==='cleared';
...
{:else}<main class="landscape-controller" class:gamepad-mode={gamepadActive} aria-label="Pill Bottle controller"><GameAudio state={state.bottle} enabled={state.audioOutput==='controllers'} rainSignal={state.rainSignal??0}/>
  {#if state.lifecycle&&!solo}<aside class="controller-scoreboard" aria-label="Scores"><strong>RACE TO 3</strong>{#each standings as player}<span class:you={player.playerId===state.playerId}>{player.name} <b>{player.score}</b>{#if player.roundPoints>0}<small>+{player.roundPoints}</small>{/if}</span>{/each}</aside>{/if}
  ...
  {#if state.lifecycle?.finished}<MatchResult title={solo?(soloCleared?'LEVEL COMPLETE':'LEVEL LOST'):state.lifecycle.matchComplete?'RACE COMPLETE':state.lifecycle.winnerId===state.playerId?'ROUND WIN':'ROUND COMPLETE'} action={solo?(soloCleared?'NEXT LEVEL':'TRY AGAIN'):state.lifecycle.matchComplete?'PLAY AGAIN':'NEXT LEVEL'} ready={state.lifecycle.readyPlayerIds.length} total={state.lifecycle.playerIds.length} disabled={state.lifecycle.readyPlayerIds.includes(state.playerId??'')} level={selectedLevel} changeLevel={(level)=>selectedLevel=level} activate={nextRound}>{#if !solo&&standings.length>0}<MatchStandings entries={standings}/>{/if}</MatchResult>{/if}
...
<style>
  ...
  .controller-scoreboard{position:absolute;z-index:6;top:max(3.2rem,calc(env(safe-area-inset-top) + 2.7rem));right:max(.55rem,env(safe-area-inset-right));display:grid;gap:min(.25rem,.7dvh);max-width:24vw;text-align:right;text-transform:uppercase;font-size:clamp(.45rem,1.6dvh,.65rem);color:var(--muted)}.controller-scoreboard span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.controller-scoreboard strong{color:var(--text)}.controller-scoreboard b,.controller-scoreboard small,.controller-scoreboard .you{color:var(--yellow)}.controller-scoreboard small{margin-left:.25rem}
