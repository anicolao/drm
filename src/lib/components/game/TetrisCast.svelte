...
<TetrisAudio enabled={true} level={progress[0]?.state.level??0}/><main><header>BLOCK STACK · ROOM {code}</header>{#if error}<h1 role="alert">{error}</h1>{:else}<section>{#each progress as player}<article class:lost={lifecycle?.terminalPlayerIds.includes(player.playerId)}><h2>{name(player.playerId)}</h2>{#if player.displayLag!==undefined}<span class="behind">BEHIND · ~{player.displayLag}t</span>{/if}<TetrisBoard state={player.state}/><p>LEVEL {player.state.level} · LINES {player.state.lines}</p><p>SCORE {player.state.score}</p>{#if !player.hashMatches}<b>REPLAY HASH MISMATCH</b>{/if}</article>{/each}</section>{#if lifecycle?.finished}<div class="result"><h1>{lifecycle.winnerId?`${name(lifecycle.winnerId)} WINS`:'GAME OVER'}</h1></div>{/if}{/if}</main>
<style>main{min-height:100vh;padding:2rem}header{border-bottom:1px solid #343746;padding-bottom:1rem;color:var(--yellow)}section{min-height:80vh;display:flex;align-items:center;justify-content:center;gap:4vw;flex-wrap:wrap}article{text-align:center}article.lost{opacity:.45}article :global(.tetris-shell){width:min(20vw,230px)}p{font-size:.7rem;color:var(--muted)}b,.behind{color:var(--pink)}.behind{display:inline-block;border:1px solid var(--pink);padding:.25rem .4rem;font-size:.55rem}.result{position:fixed;inset:0;background:#08090eee;display:grid;place-content:center}.result h1{font-size:7vw;color:var(--yellow)}</style>
