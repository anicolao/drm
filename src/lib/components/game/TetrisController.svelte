<script lang="ts">
  ...
  import { TETRIS_RULES, type TetrisInput } from '$lib/game/tetris';
  ...
  $: standings=(state.lifecycle?.playerIds??[]).map((playerId,index)=>({playerId,name:roomPlayers.find(player=>player.uid===playerId)?.displayName??(playerId===state.playerId?name:`Player ${index+1}`),score:state.lifecycle?.scores[playerId]??0})).sort((a,b)=>b.score-a.score||a.name.localeCompare(b.name));
  ...
  function start(id:string){if(id===gameId)return;gameId=id;error='';controller?.destroy();controller=createTetrisController(id,next=>{const justFinished=!state.lifecycle?.finished&&next.lifecycle?.finished;if(justFinished&&next.lifecycle?.playerIds.length===1&&next.state)selectedLevel=next.state.phase==='cleared'?Math.min(20,next.state.level):next.state.startingLevel;state=next;if(next.error)error=next.error})}
...
{:else}<main class="controller" class:gamepad-mode={gamepadActive} aria-label="Block Stack controller">
  ...
  <section class="game"><strong>{name}</strong><TetrisBoard state={state.state}/><span>LEVEL {state.state.level} / {state.state.startingLevel+TETRIS_RULES.levelsToWin} · LINES {state.state.lines}</span><span>{#if !solo}WINS {state.lifecycle?.scores[state.playerId??'']??0}/3 · {/if}SCORE {state.state.score}</span><span class="tick">TICK {state.tick}</span>{#if state.lastCommand}<small class="command-status">{state.lastCommand}</small>{/if}</section>
  ...
  {#if state.lifecycle?.finished}<MatchResult title={solo?(state.state.phase==='cleared'?'LEVEL COMPLETE':'LEVEL LOST'):state.lifecycle.winnerId===state.playerId?'ROUND WIN':state.lifecycle.matchComplete?'RACE COMPLETE':'ROUND COMPLETE'} action={solo?(state.state.phase==='cleared'?'NEXT LEVEL':'TRY AGAIN'):state.lifecycle.matchComplete?'PLAY AGAIN':'NEXT ROUND'} ready={state.lifecycle.readyPlayerIds.length} total={state.lifecycle.playerIds.length} disabled={state.lifecycle.readyPlayerIds.includes(state.playerId??'')} level={selectedLevel} changeLevel={(level)=>selectedLevel=level} activate={nextRound}>{#if !solo&&standings.length>0}<MatchStandings entries={standings}/>{/if}</MatchResult>{/if}
