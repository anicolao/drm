...
{:else}<main class="controller" class:gamepad-mode={gamepadActive} aria-label="Block Stack controller">
  ...
  <section class="game"><strong>{name}</strong><TetrisBoard state={state.state}/><span>LEVEL {state.state.level} / {state.state.startingLevel+5} · LINES {state.state.lines}</span><span>{#if !solo}WINS {state.lifecycle?.scores[state.playerId??'']??0}/3 · {/if}SCORE {state.state.score}</span><span class="tick">TICK {state.tick}</span>{#if state.lastCommand}<small class="command-status">{state.lastCommand}</small>{/if}</section>
