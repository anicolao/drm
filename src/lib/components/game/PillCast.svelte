<script lang="ts">
  ...
  $: boardWidth=Math.min(42,80/Math.max(displays.length,1));
...
<GameAudio state={audioState} enabled={audioOutput==='cast'}/><div class="display"><header><Logo compact/><div class="room-heading"><span class="live-dot" class:active={Boolean(gameId)} aria-hidden="true"></span><span>{code ? `ROOM ${code}` : 'NO ROOM'}</span></div></header><main>{#if error}<section class="message"><h1>DISPLAY UNAVAILABLE</h1><p role="alert">{error}</p></section>{:else if !loaded}<p role="status">Loading room…</p>{:else if !gameId}<section class="message waiting"><div><p class="eyebrow">{ruleset === 'tetris' ? 'BLOCK STACK' : 'COLOR CURE'}</p><h1>WAITING FOR A GAME</h1><p>Scan to join as a controller, or enter room code <strong>{code}</strong>. Every board shown here is reconstructed from immutable controller commands.</p></div>{#if joinHref}<JoinQr href={joinHref} label="Scan to join" />{/if}</section>{:else if displays.length===0}<section class="message"><p class="eyebrow">COLOR CURE · GAME ACTIVE</p><h1>WAITING FOR PLAYERS</h1><p role="status">Controllers will appear after their first progress command.</p></section>{:else}<section class="arena" aria-label="Shared game display" style={`--board-width:${boardWidth}vw`}>{#if lifecycle&&!solo}<aside class="scoreboard"><strong>RACE TO 3</strong>{#each standings as player}<span>{player.name} <b>{lifecycle.scores[player.playerId]??0}</b>{#if lifecycle.roundPoints[player.playerId]} <small>+{lifecycle.roundPoints[player.playerId]}</small>{/if}</span>{/each}</aside>{/if}{#if lifecycle?.finished}<MatchResult title={solo?(lifecycle.winnerId?'LEVEL COMPLETE':'LEVEL LOST'):lifecycle.matchComplete?'RACE COMPLETE':lifecycle.winnerId?`${standings.find(player=>player.playerId===lifecycle?.winnerId)?.name??'PLAYER'} WINS THE ROUND`:'ROUND DRAW'} interactive={false} ready={lifecycle.readyPlayerIds.length} total={lifecycle.playerIds.length}>{#if !solo&&standings.length>0}<MatchStandings entries={standings.map(player=>({playerId:player.playerId,name:player.name,score:lifecycle?.scores[player.playerId]??0}))}/>{/if}</MatchResult>{/if}{#each displays as player (player.playerId)}<CastPlayerFrame name={player.name} lost={lifecycle?.terminalResults[player.playerId]==='lost'} lag={player.displayLag} hashMatches={player.hashMatches}><PillBottle state={player.state}/><p>level {player.state.level} · {player.state.viruses} viruses</p><p>speed {gravityTicksForState(player.state)} ticks · display tick {player.tick}</p>{#if lifecycle?.terminalResults[player.playerId]==='cleared'}<strong class="countdown">LEVEL CLEAR{#if !solo} · ROUND WIN{/if}</strong>{:else if lifecycle?.terminalPlayerIds.includes(player.playerId)}<strong class="eliminated">ELIMINATED</strong>{/if}{#if player.displayLag!==undefined}<p class="tick-detail">controller is significantly behind</p>{/if}</CastPlayerFrame>{/each}</section>{/if}</main></div>

<style>.display{min-height:100vh;padding:1rem 4vw}header{display:flex;justify-content:space-between;border-bottom:1px solid #343746;padding-right:9rem;padding-bottom:.5rem}.room-heading{display:flex;align-items:center;gap:.55rem;font-weight:700}.live-dot{width:.65rem;height:.65rem;border-radius:50%;background:var(--yellow)}.live-dot.active{background:var(--cyan)}main{min-height:82dvh;display:grid;place-content:center}.message{text-align:center}.message.waiting{display:flex;align-items:center;justify-content:center;gap:clamp(2rem,7vw,7rem);text-align:left}.message h1{font:900 clamp(2.5rem,7vw,6rem) 'Arial Black'}.message p:last-child{color:var(--muted);max-width:42rem;line-height:1.7}.arena{width:92vw;display:flex;justify-content:center;align-items:start;gap:clamp(.5rem,1vw,1rem);flex-wrap:nowrap}.arena :global(article){min-width:0;flex:0 1 auto}.arena :global(p){margin:.25rem;color:var(--muted);text-transform:uppercase;font-size:.65rem}.arena :global(.bottle-shell){width:min(var(--board-width),40dvh,460px)}.countdown{display:block;color:var(--yellow);font-size:.8rem}.eliminated{color:var(--pink)}.scoreboard{position:fixed;top:4.5rem;right:4vw;display:grid;gap:.35rem;text-align:right;text-transform:uppercase}.scoreboard span{color:var(--muted)}.scoreboard b,.scoreboard small{color:var(--yellow)}@media(max-width:700px){header{padding-right:5.5rem}.message.waiting{flex-direction:column;text-align:center}.scoreboard{position:static;margin-bottom:.5rem}.arena{padding-top:.5rem}}</style>
