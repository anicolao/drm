<script lang="ts">
 import{Canvas,T}from'@threlte/core';import StaxTile from'./StaxTile.svelte';import type{StaxState}from'$lib/game/stax';
...
<div class="scene" class:compact aria-label={label} data-tick={state.tick} data-phase={state.phase} data-paddle-lane={state.paddleLane} data-paddle-count={state.paddle.length} data-ramp-count={state.ramp.length} data-column-counts={state.columns.map(column=>column.length).join(',')} data-score={state.score} data-ramp-direction="far-to-player" data-paddle-position="player-edge" data-bin-position="below-paddle">
 <Canvas dpr={[1,2]}>
  ...
  <T.Mesh position={[0,-.08,5.15]} receiveShadow><T.BoxGeometry args={[6.3,.18,3.35]}/><T.MeshStandardMaterial color="#0b1020" metalness={.22} roughness={.42}/></T.Mesh>
  {#each Array(6) as _,i}<T.Mesh position={[(i-2.5)*1.15,.025,5.15]}><T.BoxGeometry args={[.025,.025,3.2]}/><T.MeshBasicMaterial color="#263d68"/></T.Mesh>{/each}
  {#each Array(6) as _,i}<T.Mesh position={[0,.025,3.55+i*.64]}><T.BoxGeometry args={[5.75,.025,.025]}/><T.MeshBasicMaterial color="#263d68"/></T.Mesh>{/each}
  {#each state.columns as column,c}{#each column as color,r}<StaxTile {color} position={[x(c),.2,6.43-r*.64]} scale={[.9,.9,.58]} glow={state.lastClearCells.some(cell=>cell.column===c&&cell.row===r)?.65:.28}/>{/each}{/each}
