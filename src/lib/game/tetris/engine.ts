...
function spawn(state:TetrisState){state.softDrop=false;ensureNext(state);const kind=state.next.shift()!;ensureNext(state);const active:TetrisActive={id:state.nextId++,kind,row:TETRIS_HIDDEN_ROWS-1,col:3,orientation:0,groundedTicks:0,lockResets:0};state.active=fits(state,active)?active:null;if(!state.active)state.phase='lost'}
export function createTetris(seed:number,level=0):TetrisState{const state:TetrisState={rulesVersion:'tetris/2',tick:0,board:Array(TETRIS_HEIGHT*TETRIS_WIDTH).fill(null),active:null,bag:[],next:[],rng:seed>>>0||1,nextId:1,lines:0,score:0,startingLevel:level,level,phase:'playing',softDrop:false};ensureNext(state);spawn(state);return state}
...
export function hashTetris(state:TetrisState){const text=JSON.stringify(state);let hash=2166136261;for(let i=0;i<text.length;i++){hash^=text.charCodeAt(i);hash=Math.imul(hash,16777619)}return`t2-${(hash>>>0).toString(16).padStart(8,'0')}`}
