...
function spawn(state:TetrisState){ensureNext(state);const kind=state.next.shift()!;ensureNext(state);const active:TetrisActive={id:state.nextId++,kind,row:TETRIS_HIDDEN_ROWS-1,col:3,orientation:0,groundedTicks:0,lockResets:0};state.active=fits(state,active)?active:null;if(!state.active)state.phase='lost'}
