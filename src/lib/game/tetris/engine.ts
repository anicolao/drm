...
export function createTetris(seed:number,level=0):TetrisState{const state:TetrisState={rulesVersion:'tetris/1',tick:0,board:Array(TETRIS_HEIGHT*TETRIS_WIDTH).fill(null),active:null,bag:[],next:[],rng:seed>>>0||1,nextId:1,lines:0,score:0,startingLevel:level,level,phase:'playing',softDrop:false};ensureNext(state);spawn(state);return state}
