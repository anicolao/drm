...
export interface TetrisState { rulesVersion:'tetris/1'; tick:number; board:Array<Tetromino|null>; active:TetrisActive|null; bag:Tetromino[]; next:Tetromino[]; rng:number; nextId:number; lines:number; score:number; startingLevel:number; level:number; phase:'playing'|'lost'; softDrop:boolean }
