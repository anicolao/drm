<script lang="ts">
  ...
  let code = ''; let ruleset: 'tetris' | 'doctor' | 'quarry' = 'tetris'; let players: RoomPlayer[] = [];
  ...
  async function choose(next: 'tetris' | 'doctor' | 'quarry') {
...
<main><section class="card"><p class="eyebrow">Game configuration</p><div class="modes"><button disabled={Boolean(activeGameId)||choosing} class:chosen={ruleset==='tetris'} on:click={()=>choose('tetris')}>▦<strong>BLOCK STACK</strong><small>Tetris-style rules</small></button><button disabled={Boolean(activeGameId)||choosing} class:chosen={ruleset==='doctor'} on:click={()=>choose('doctor')}>●<strong>COLOR CURE</strong><small>Dr. Mario-style rules</small></button><button disabled={Boolean(activeGameId)||choosing} class:chosen={ruleset==='quarry'} on:click={()=>choose('quarry')}>◆<strong>QUARRY MATCH</strong><small>Match-three puzzle race</small></button></div>{#if activeGameId}<p class="notice active" role="status">Game started · 60 ticks/second</p>{:else}<div class="starts"><button disabled={starting || choosing || players.length === 0} on:click={()=>startGame('play')}>{starting ? 'Starting…' : 'Play'}</button><button class="secondary" disabled={starting || choosing || !hasControllerPlayer} on:click={()=>startGame('cast')}>{starting ? 'Starting…' : 'I am the TV'}</button></div><p class="notice"><strong>Play</strong> uses this device as the host's controller. <strong>I am the TV</strong> keeps this device on the shared display for the other players.</p>{/if}</section>
