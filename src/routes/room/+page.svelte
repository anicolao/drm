<script lang="ts">
  ...
  async function startGame() {
    ...
    starting = true; error = '';
    try { activeGameId = await startPillBottleGame(roomId, players); }
    catch (cause) { error = cause instanceof Error ? cause.message : String(cause); }
...
<main><section class="card"><p class="eyebrow">Game configuration</p><div class="modes"><button disabled={Boolean(activeGameId)} class:chosen={ruleset==='tetris'} on:click={()=>choose('tetris')}>▦<strong>BLOCK STACK</strong><small>Tetris-style rules</small></button><button disabled={Boolean(activeGameId)} class:chosen={ruleset==='doctor'} on:click={()=>choose('doctor')}>●<strong>COLOR CURE</strong><small>Dr. Mario-style rules</small></button></div>{#if activeGameId}<p class="notice active">Controller game started · 60 ticks/second</p>{:else}<button class="start" disabled={starting || ruleset !== 'doctor' || players.length === 0} on:click={startGame}>{starting ? 'Starting…' : 'Start Color Cure controller'}</button><p class="notice">This initial slice starts the real tick clock and input stream. Bottle physics are not implemented yet.</p>{/if}</section>
