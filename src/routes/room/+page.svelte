<script lang="ts">
  ...
  let roomId = ''; let activeGameId = ''; let loading = true; let starting = false; let error = '';
  ...
  async function startGame(destination: 'play' | 'cast') {
    if (ruleset !== 'doctor') { error = 'The initial gameplay implementation supports Color Cure only.'; return; }
    starting = true; error = '';
    try { activeGameId = await startPillBottleGame(roomId, players, destination === 'play' ? 'player' : 'display'); await goto(`${base}/${destination}?code=${code}`); }
    catch (cause) { error = cause instanceof Error ? cause.message : String(cause); }
...
<main><section class="card"><p class="eyebrow">Game configuration</p><div class="modes"><button disabled={Boolean(activeGameId)} class:chosen={ruleset==='tetris'} on:click={()=>choose('tetris')}>▦<strong>BLOCK STACK</strong><small>Tetris-style rules</small></button><button disabled={Boolean(activeGameId)} class:chosen={ruleset==='doctor'} on:click={()=>choose('doctor')}>●<strong>COLOR CURE</strong><small>Dr. Mario-style rules</small></button></div>{#if activeGameId}<p class="notice active">Color Cure game started · 60 ticks/second</p>{:else}<div class="starts"><button disabled={starting || ruleset !== 'doctor' || players.length === 0} on:click={()=>startGame('play')}>{starting ? 'Starting…' : 'Play'}</button><button class="secondary" disabled={starting || ruleset !== 'doctor' || !hasControllerPlayer} on:click={()=>startGame('cast')}>{starting ? 'Starting…' : 'I am the TV'}</button></div><p class="notice"><strong>Play</strong> uses this device as the host's controller. <strong>I am the TV</strong> keeps this device on the shared display for the other players.</p>{/if}</section>
