<script lang="ts">
  ...
  import { getRoom, getRoomPlayers, updateRoomRuleset, type RoomPlayer } from '$lib/firebase/rooms';
  ...
  async function load() {
    ...
    try {
      ...
      const room = await getRoom(code); ruleset = room.ruleset; players = await getRoomPlayers(room.id);
...
<section class="card roster"><p class="eyebrow">Joined players · {players.length}</p>{#each players as player}<div><span class="avatar">●</span><strong>{player.role === 'host' ? 'You' : 'Joined player'}</strong><small>{player.role.toUpperCase()}</small></div>{/each}{#if players.length === 0}<p>No players have joined.</p>{/if}</section></main>{/if}
</div>
