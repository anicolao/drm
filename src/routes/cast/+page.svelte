...
<div class="display"><header><Logo compact/><span>{code ? `ROOM ${code}` : 'NO ROOM'}</span></header><main>{#if error}<section class="message"><h1>DISPLAY UNAVAILABLE</h1><p role="alert">{error}</p></section>{:else if !loaded}<p>Loading room…</p>{:else if !gameId}<section class="message"><p class="eyebrow">{ruleset === 'tetris' ? 'BLOCK STACK' : 'COLOR CURE'}</p><h1>WAITING FOR A GAME</h1><p>The shared display reconstructs every board by replaying immutable RTDB commands.</p></section>{:else if displays.length===0}<section class="message"><p class="eyebrow">COLOR CURE · GAME ACTIVE</p><h1>WAITING FOR PLAYERS</h1><p>Joined controllers will appear when they publish real game progress.</p></section>{:else}<section class="arena" aria-label="Shared game display">{#each displays as player (player.playerId)}<article><h2>{player.name}</h2><PillBottle state={player.state}/><p>{player.state.viruses} viruses · tick {player.tick}</p>{#if !player.hashMatches}<p class="sync-error">Replay hash mismatch</p>{/if}</article>{/each}</section>{/if}</main></div>
