...
{#if error}<main><h1 role="alert">{error}</h1></main>{:else if loading}<main><p role="status">Loading display…</p></main>{:else if !activeGameId}<main><section><div><p class="eyebrow">{ruleset==='tetris'?'BLOCK STACK':ruleset==='quarry'?'QUARRY MATCH':ruleset==='canopy'?'CRYSTAL CANOPY':ruleset==='stax'?'STAX':'COLOR CURE'}</p><h1>WAITING FOR A GAME</h1><p>Scan to join as a controller, or enter room code <strong>{code}</strong>. Every board shown here is reconstructed from immutable controller commands.</p></div>{#if joinHref}<JoinQr href={joinHref} label="Scan to join"/>{/if}</section><header>ROOM {code}</header></main>{:else if ruleset==='tetris'}<TetrisCast/>{:else if ruleset==='doctor'}<PillCast/>{:else if ruleset==='quarry'}<QuarryCast/>{:else if ruleset==='canopy'}<QuarryCast variant="canopy"/>{:else if ruleset==='stax'}<StaxCast/>{/if}
