<script lang="ts">
  ...
  onMount(async()=>{code=new URL(window.location.href).searchParams.get('code')??'';try{if(!code)throw new Error('A room code is required.');if(!firebaseConfigured)throw new Error('Firebase configuration is required.');await joinRoom(code);joined=true;}catch(cause){error=cause instanceof Error?cause.message:String(cause)}});
</script>
