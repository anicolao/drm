...
{:else}<main class="landscape-controller" aria-label="Pill Bottle controller"><GameAudio state={state.bottle} enabled={state.audioOutput==='controllers'}/>
...
<style>
  ...
  .session{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);display:grid;text-align:center;gap:min(.2rem,.6dvh);color:var(--muted);text-transform:uppercase;font-size:clamp(.42rem,1.45dvh,.58rem);max-height:96dvh}.session :global(.bottle){width:min(15vw,36dvh,112px)}.session strong{color:var(--text);font-size:clamp(.58rem,2dvh,.8rem)}.session .tick{color:var(--yellow);font-size:clamp(.58rem,2dvh,.8rem)}
