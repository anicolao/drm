...
import{parseStaxRecord,parseStaxStart}from'../../src/lib/protocol/stax.ts';
...
test('Stax protocol accepts rules and commands but rejects materialized state',()=>{const start={type:'game/started',ruleset:'stax',rulesVersion:'stax/1',seed:1,tickRate:60,round:0,matchId:'m',hostUid:'h',roomId:'r',audioOutput:'controllers',members:{p:true},players:{p:{seat:0,level:2}},settings:{width:5,height:5,targetWins:3,matchRounds:3},scores:{p:0},serverTime:1}as const;assert.equal(parseStaxStart(start).rulesVersion,'stax/1');assert.throws(()=>parseStaxStart({...start,columns:[]}));assert.equal(parseStaxRecord('a',{type:'input/place',playerId:'p',epochId:'e',clientSeq:1,tick:4,serverTime:1}).type,'input/place');assert.throws(()=>parseStaxRecord('a',{type:'input/place',playerId:'p',epochId:'e',clientSeq:1,tick:4,columns:[]}))});

test('three-round ranking awards N minus rank and preserves exact ties',()=>{const ids=['a','b','c'];const round=deriveStaxLifecycle(ids,{a:0,b:0,c:0},[{playerId:'a',result:'cleared',tick:50,score:900,misses:1,seat:0},{playerId:'b',result:'lost',tick:60,score:1200,misses:5,seat:1}],[],0);assert.equal(round.finished,true);assert.deepEqual(round.roundPoints,{a:2,c:1,b:0});assert.equal(round.matchComplete,false);const last=deriveStaxLifecycle(ids,{a:2,b:3,c:0},[{playerId:'a',result:'lost',tick:50,score:0,misses:5,seat:0},{playerId:'b',result:'lost',tick:50,score:0,misses:5,seat:0}],[],2);assert.equal(last.matchComplete,true);assert.deepEqual(last.winnerIds,['a','b'])});
